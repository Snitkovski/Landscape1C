...
Все одним действием: на macOS — `./start.command` (можно двойным кликом), на Windows — `start.cmd`. Поднимет локальный сервер и откроет редактор; сайт — на `http://127.0.0.1:8123/`.

Из зависимостей — только **Python 3** и **Node.js**, ставить пакеты не нужно (ни `pip install`, ни `npm install`). Node нужен кнопкам «Сохранить» / «Собрать dist» и скриптам проверки; просто посмотреть сайт можно и без него.
